...
export default function BudgetSummary({ budgets }: BudgetSummaryProps) {
    ...
    const stats = [
        ...
        },
        {
            ...
            icon: TrendingDown,
            color: overallPct >= 100 ? "text-red-600" : overallPct >= 80 ? "text-amber-600" : "text-blue-600",
            bg: overallPct >= 100 ? "bg-red-50" : overallPct >= 80 ? "bg-amber-50" : "bg-blue-50",
            ring: overallPct >= 100 ? "ring-red-200" : overallPct >= 80 ? "ring-amber-200" : "ring-blue-200",
        },
        {
            label: "Over Budget",
            ...
            icon: AlertTriangle,
            color: overCount > 0 ? "text-red-600" : "text-slate-400",
            bg: overCount > 0 ? "bg-red-50" : "bg-slate-50",
            ring: overCount > 0 ? "ring-red-200" : "ring-slate-200",
        },
    ];
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {stats.map(({ label, value, sub, icon: Icon, color, bg, ring }) => (
                <div
                    key={label}
                    className="flex items-center gap-4 rounded-2xl bg-white/70 border border-white/60 backdrop-blur-xl shadow-sm shadow-slate-200/50 px-5 py-4"
                >
